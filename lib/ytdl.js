...
function requestAPI(hostname, path, apiKey) {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const [videoData, musicData] = await Promise.all([
                requestAPI('youtube-mp4-downloader.p.rapidapi.com', `/mp4?url=${encodeURIComponent(url)}`,  process.env.rapid_api_token),
                requestAPI('yt-search-and-download-mp3.p.rapidapi.com', `/mp3?url=${encodeURIComponent(url)}`,  process.env.rapid_api_token)
            ]);
